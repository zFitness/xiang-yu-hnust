<template>
  <div class="chat-container">
    <!-- 导航栏 -->
    <van-nav-bar class="nav-bar" @click-left="onClickLeft" left-text="返回"
                 left-arrow>
      <!-- 标题 -->
      <template #title>
      ...
      <van-list v-model="loading" :finished="finished">
        <div class="message-item" v-for="(item,index) in list" :key="index">
          ...
          <div class="message-item-content" v-if="item.id != myId">
            <van-image class="avatar" round width="1rem" height="1rem"
                       :src="item.avatar" @click="showMatchUserInfo = true" />
    ...
    <div class="control-bottom">
      <div class="control">
        ...
        <van-field class="input" v-model="msg" placeholder="输入消息"
                   @keyup.enter="send" :disabled="disabled"
                   @click="closeTools" />
        <van-icon class="emoji-btn" name="smile-o" size="28" @click="openEmoji"
                  style="margin-right:8px" />
        <van-button class="send-btn" size="small" type="primary" @click="send"
                    :color="disabled?'#D9DDDD':''">发送</van-button>
      </div>
      <!-- 表情框 -->
      <div class="control-emoji" v-if="emojiShow">
        <div v-for="(item,index) in emoji" :key="index" @click="addEmoji(item)">
          {{ item }}</div>
      </div>

      <!-- 工具框 -->
      <div class="control-tools" v-if="emojiTools">
        <tools-img v-if="userInfo != null" :userInfo="userInfo"
                   @send="handleImgSend" />
        ...
        <tools-music v-if="userInfo != null" :userInfo="userInfo"
                     @send="handleMusicSend" />
        <div class="tools-item">
          <div class="tools-item-img">
            <van-icon name="photo-o" size="22px" />
          </div>
      ...
      <div class="match-user-info">
        ...
        <div>性别：
          <van-icon class-prefix="my-icon"
                    :name="matchUser.gender=='男'?'nan':'nv'" />
        </div>
      </div>
    </van-dialog>
